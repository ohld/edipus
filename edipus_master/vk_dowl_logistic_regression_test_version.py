...
class Calculate_thread(QtCore.QThread):
    ...
    def run(self):
        ...
        # self.emit(QtCore.SIGNAL("logistic_regression_trained")) # was: svm_regression_trained
        while True:
            ok,img = cap.read()
            if img != None:
            ...
            if (self.image is not None):
                ...
                id = self.lr.predict_proba(rep)[1][0]   # now id -> probability of id == 1.
...
def trainLR(Faces):
    ...
    lr = GridSearchCV(LR(C=1), param_grid, cv=5, n_jobs=2, scoring='roc_auc').fit(rep, identities) # n_jobs param is for parallel computation, try differen values
    return le
